...
以tick开头的触发器可以配置触发间隔, 它的的单位是tick(50ms), 不填写的话默认为10tick, 支持解析动作变量及即时声明节点, 如:
...
如果你想在判断condition之前执行一些动作, 可以使用pre配置项:

```yaml
test1:
  left:
    consume:
      # consume完成后物品数量就变了, 因此在pre存储一下物品数量
      pre: 'setGlobal: amount <amount::>'
      amount: <amount>
    sync:
