...
Messages:
  ...
  # 脚本出错提示信息
  invalidScript: '§e[NI] §6载入 {script} 脚本时出现错误, 报错如下:'
  # 执行扩展出错提示信息
  expansionError: '§e[NI] §6执行 {expansion} 脚本的 {function} 函数时出现错误, 报错如下:'
  # 物品耐久耗尽提示信息
  brokenItem: '§e物品耐久已耗尽'
  # 物品更新提示信息
  legacyItemUpdateMessage: '§e[NI] §6你所持有的旧物品 §r{name} §6已被更新'
  # 扩展构建插件提示信息
  buildExpansionMessage: '§bNeige§3Items §7> §a已将 §f{expansion} §a打包为 §f{file}'
  # 一键保存MM物品时, MM物品生成时出现错误的提示信息
  invalidMMItem: §bNeige§3Items §7> §7生成MM物品 §f{itemID} §7时出现错误
  # 计算节点输入无效的取整模式时的提示信息
  invalidRoundingMode: §bNeige§3Items §7> §7未知的取整模式 §f{mode}
  # 计算节点输入无法计算的数学公式时的提示信息
...
ExampleItem4:
  material: STONE
  name: "&f%PoemSuits_nbt_PoemSuits.id%"
  lore:
  - '&f物品使用次数: %PoemSuits_charge%/%PoemSuits_maxCharge%'
  options:
    charge: 10
...
DurabilityTest1:
  ...
  # 发包显示(创造模式不解析)
  name: "%PoemSuits_durability%/%PoemSuits_maxdurability%"
  lore:
    - "%PoemSuits_itembreak_耐久耗尽不损坏_耐久耗尽损坏%"
  options:
...
function enable() {
    ...
    const SectionManager = Packages.pers.neige.PoemSuits.manager.SectionManager.INSTANCE
    const CustomSection = Packages.pers.neige.PoemSuits.section.impl.CustomSection
...
```
## Expansions/CustomItemEditor.js
